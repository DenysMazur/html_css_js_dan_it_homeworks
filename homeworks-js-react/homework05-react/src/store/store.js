...
import { ADD_PRODUCT_TO_CART, ADD_TO_FAVORITES_LIST, CLEAN_CART, REMOVE_FROM_FAVORITES_LIST, REMOVE_PRODUCT_FROM_CART } from './types';
...
const cartSyncMiddleware = store => next => action => {
  const result = next(action)
  if ([ADD_PRODUCT_TO_CART, REMOVE_PRODUCT_FROM_CART, CLEAN_CART].includes(action.type)) {
